...
class ESMetrics < Sensu::Plugin::Metric::CLI::Graphite
  ...
  option :stats_enabled,
    description: 'Enable statistics separated by comma',
    short: '-e STATS',
    long: '--enable-stats STATS',
    proc: Proc.new { |names| Set.new(names.split(',')) },
    default: Set.new(['_all']) # means all
  ...
  def stats_enabled?(s)
    config[:stats_enabled].include? '_all' or config[:stats_enabled].include? s
  end

  def run
    stats_enabled_str = config[:stats_enabled].to_a.join(',')
    stats = get_es_resource("_nodes/_local/stats/#{stats_enabled_str}")
    ...
    stats_enabled? 'transport' and node['transport'].each do |k, v|
      unless (k =~ /(_size$)/)
        metrics["transport.#{k}"] = v
      end
    end

    if stats_enabled? 'http'
